...
| :---: | :--- | :--- | :--- | :--- | :--- |
| [x] | **1.1.1** | **As a New Visitor**, I want to **register for an account** so that I can buy items and track my orders later. | High | 5 | 1. User can enter First Name, Last Name, Email, Password, Confirm Password, Phone (optional), and Address (optional).<br>2. System validates that first name and last name are provided (required fields).<br>3. System validates email format and prevents duplicate email registrations.<br>4. System validates password strength (minimum 8 characters).<br>5. System validates that Confirm Password matches Password.<br>6. Upon success, user is redirected to Login page with confirmation message and new account can log in immediately. |
| [x] | **1.1.2** | **As a Registered Customer**, I want to **log in with my credentials** so that I can access my saved details and complete purchases. | High | 3 | 1. A login Modal/Dialog is triggered by clicking the "Login" navbar button.<br>2. User enters Email and Password in the modal form.<br>3. System validates credentials against the database.<br>4. On success, the system stores the JWT Token in LocalStorage, updates AuthContext to authenticate the user, closes the modal, and shows a success Toast.<br>5. On failure, a destructive Toast displays "Invalid email or password".<br>6. Modal includes a link to switch to the Registration page. |
...
| Status | ID | User Story | Priority | Est. (Pts) | Acceptance Criteria |
| :---: | :--- | :--- | :--- | :--- | :--- |
| [x] | **1.4.1** | **As a Ready-to-Buy Customer**, I want to **enter my shipping address** so that the store knows where to send my electronics. | High | 3 | 1. Checkout Step 1 displays the logged-in customer's full name (First Name + Last Name) as a read-only label based on session data.<br>2. Form fields: Address, City, State/Province, Zip/Postal Code, Country.<br>3. If user is logged in, fields pre-fill from their Profile (if saved).<br>4. Form validation requires all fields to be completed before proceeding.<br>5. The system must concatenate Address, City, State/Province, Zip/Postal, and Country into a single `clientAddress` string for the backend payload.<br>6. "Next" button advances to payment step. |
| [x] | **1.4.2** | **As a Customer**, I want to **simulate a payment** so that I can complete the order process without using real money. | High | 5 | 1. Checkout Step 2 displays payment method selection (Credit Card, PayPal, etc.).<br>2. Credit Card method includes: Name on Card, 16-digit Card Number, Exp Date (MM/YY), and 3-digit CVV fields.<br>3. PayPal method renders a mock "Log in to PayPal" button that acts as the continue button. When clicked, it generates a mock token (e.g., `mock_paypal_123`) and instantly advances to Step 3.<br>4. System validates card format (16 digits, exp date, CVV).<br>5. Form includes a "Continue to Review" button. On valid submit, the system generates a mock token (e.g., `mock_cc_123`), saves it to local checkout state, and advances to Step 3. |
| [x] | **1.4.3** | **As a Customer**, I want to **review my order before final submission** so that I can verify all details are correct. | High | 3 | 1. Checkout Step 3 displays Order Summary with cart items in a read-only visual format (Product image, price, quantity, row amount) matching the cart interface.<br>2. An "Edit Cart" link is provided to redirect the user back to the main `/cart` page to make changes.<br>3. User can see shipping address and total.<br>4. User can edit cart quantities or go back to address step via "Edit" links.<br>5. The "Place Order" button is disabled if the cart is empty.<br>6. "Place Order" button submits the purchase transaction.<br>7. **SECURITY: Backend independently fetches current prices from `lpa_stock` table (never trusts frontend prices) to prevent client-side tampering of order totals.** |
