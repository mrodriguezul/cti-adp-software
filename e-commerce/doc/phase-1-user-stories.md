...
| **1.5.3** | **As a Warehouse Staff**, I want to **update the order status to "Shipped"** so that the system tracks fulfillment. | Medium | 2 | 1. Admin can change `lpa_invoices.status` from "P" (Paid) to "S" (Shipped).<br>2. The status update is saved to the database. |